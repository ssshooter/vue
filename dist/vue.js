...
; (function (global, factory) {
  ...
  function pushTarget(_target) {// _target是一个watcher
    if (Dep.target) {
  ...
  var Observer = function Observer(value) {
    ...
    this.vmCount = 0
    // def(对象，键，值，可枚举)
    // 把一个值的__ob__定义为一个Obersver
    def(value, '__ob__', this)
    if (Array.isArray(value)) {
      // 如果观察对象是数组
      // hasProto的作用是检查是否可以使用__proto__
  ...
  function observe(value, asRootData) {
    ...
    if (hasOwn(value, '__ob__') && value.__ob__ instanceof Observer) {
      ...
      // 需转换 且 不是服务器渲染 且 这是数组或者PlainObject 且 可以新增属性 且 不是Vue对象
  ...
  function mountComponent(vm, el, hydrating) {
    ...
    callHook(vm, 'beforeMount')  // 以下开始挂载

    var updateComponent
    /* istanbul ignore if */
    if ('development' !== 'production' && config.performance && mark) { // performance设置为 true 以在浏览器开发工具中启用对组件初始化、编译、渲染和打补丁的性能追踪。只适用于开发模式和支持 performance.mark API 的浏览器上。
   ...
   * cb 回调函数
   * option 选项
   * 计算属性option自带lazy:true
   */
  var Watcher = function Watcher(vm, expOrFn, cb, options) {
    this.vm = vm
    // vm._watchers可以看到全部watcher
  ...
  Watcher.prototype.get = function get() {
    pushTarget(this) // 粗略来说就是向dep target赋值
    var value
    var vm = this.vm
    try {
  ...
  /**
   * 递归遍历一个对象，唤起里面所有getter
   * 所以所有嵌套属性都能作为“deep”依赖被收集
  ...
  function initData(vm) {
    ...
    // observe data 发布数据
    observe(data, true /* asRootData */) // 是RootData的话vmCount++
    // 递归响应化
  }
  ...
  function createWatcher(vm, keyOrFn, handler, options) {
    if (isPlainObject(handler)) { //是对象就抽离handler
      options = handler
      handler = handler.handler
    }
    if (typeof handler === 'string') {
      handler = vm[handler] // 是字符串就找实例的方法
    }
    // options 
    // deep -> 深度 watcher 
  ...
  /* provide 和 inject 主要为高阶插件/组件库提供用例。并不推荐直接用于应用程序代码中 详见官方文档 */
  ...
  /*
   * Vue 推荐在绝大多数情况下使用 template 来创建你的 HTML。然而在一些场景中，
 ...
 */
  ...
  function checkEvent(exp, text, errors) {
    ...
    if (keywordMatch && stipped.charAt(keywordMatch.index - 1) !== '$') {
      errors.push(
        '避免使用JavaScript一元操作符作property name: ' +
        '"' +
        keywordMatch[0] +
        '"' +
        text.trim()
  ...
  var createCompiler = createCompilerCreator(function baseCompile(
    ...
    var code = generate(ast, options) // 生产渲染代码
    console.log({
      ast: ast,
      render: code.render,
